...
Descrip Creation_descripteur_audio(const char *nomfichier,long *k,long n,int m){
 ...
 // mon_descripteur.identifiant = nomfichier; // initialisation de l'identifiant du descripteur 
  ...
  FILE * fic;
  fic = fopen("../../documents/audio/base_descripteur_audio.txt","a"); // Ouverture du fichier pour base_descirpteur_audio.txt pour enregistrer les descripteurs crées.
...
Descrip * indexation_automatique(Descrip * base_descript_audio,int *nbr_descripteur,int n ,int m){
  ...
  FILE * fp; 
  FILE * f;
  f=fopen("../../documents/audio/liste_base_audio.txt","a+"); // Pour creer ou mettre a jour "liste_base_audio.txt" le fichier qui contient la liste des fichiers indexer
  //fp=fopen("audio_paths.txt","r"); //Ouvre le fichier contenant les chemins absolue qui doivent etre indexer
  fp = popen("find $PWD -type f -name \"*.bin\"","r");// Ces deux commandes servent a trouver les fichiers audio .bin dans "Audio_data_base"
